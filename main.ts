...
import { COURSE_WITH_LARGEST_WORKLOAD, BURNT_OUT, SENIORITIS, P_ENG, CONSIDERED_SWITCH_OUT_OF_SE, PROGRAM_TO_SWITCH_TO, CONSIDERED_DROPPING_OUT_OF_UNI, START_AGAIN_PROGRAM, HAPPY_WITH_CHOICE, } from './data/experience';
...
function renderExperience(options) {
  renderHorizontalBarChat(d3.select('#largest-workload'), COURSE_WITH_LARGEST_WORKLOAD, options.width, 280, true);
  renderPieChart(d3.select('#p-eng'), P_ENG, options.width * 0.75, options.width * 0.75);
  renderPieChart(d3.select('#switch-out-se'), CONSIDERED_SWITCH_OUT_OF_SE, options.width * 0.75, options.width * 0.75);
  renderPieChart(d3.select('#program-switch-to'), PROGRAM_TO_SWITCH_TO, options.width * 0.75, options.width * 0.75);
  renderPieChart(d3.select('#drop-out-waterloo'), CONSIDERED_DROPPING_OUT_OF_UNI, options.width * 0.75, options.width * 0.75);
  renderHorizontalBarChat(d3.select('#redo-uni'), START_AGAIN_PROGRAM, options.width, 280, true);
  ...
  renderHorizontalBarChat(d3.select('#happy'), HAPPY_WITH_CHOICE, options.width, 280, true);
}
